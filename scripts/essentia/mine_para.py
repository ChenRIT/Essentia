...
from fsa import create_fsa, process_sents, find_verb_paraphrases, find_phrase_paraphrases, get_repr_nd_text
...
def gene_phrase_paras_first_chunk_single_file(input_file, output_dir, origin_sents):
    ...
    with open(input_file) as ifile:
        for line in ifile:
            chunks = divide_sent_by_prep(line)
...
def gene_phrase_paras_vps_single_file(input_file, output_dir):
    ...
    logging.info("Creating the FSA representing the input sentences...")
    fsa, sent_num, origin_sents, _ = create_fsa_from_file(input_file)
    logging.info("FSA recreated!")
    # Find verb phrases out of the paraphrases
    logging.info("Searching for paraphrases...")
    word_to_para = find_verb_paraphrases(fsa, origin_sents)
    logging.info("Storing the mined paraphrases at {}...".format(output_dir))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dir", default="./datasets/", type=str, help="The input directory of sentence files.")
    parser.add_argument("--output", default="./snips_paras/", type=str, help="The output directory containing domain-specific paraphrases.")
    args = parser.parse_args()

    if not os.path.exists(args.output):
        os.makedirs(args.output)

    gene_vp_para_dir(args.output, args.dir)
